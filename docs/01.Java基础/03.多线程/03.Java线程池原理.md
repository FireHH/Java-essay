...
![线程池原理](https://fire-repository.oss-cn-beijing.aliyuncs.com/2024/1231/threadPool.png)
...
`Executors`内部是现场也是通过`ThreadPoolExecutor`来实现的，只是对`ThreadPoolExecutor`进行了封装，提供了一些工厂方法。
