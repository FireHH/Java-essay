...
下载地址：https://www.rabbitmq.com/install-windows-manual.html
...
//生产者，正在往 topic 交换机发送消息，队列绑定了不同路由规则，交换机会使用 nr.topic.b 作为路由规则来发送
// 用 nr.topic.b 和 nr.topic.#作为路由 key 的队列都能收到消息
